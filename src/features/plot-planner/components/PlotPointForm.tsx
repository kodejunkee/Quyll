...
export function PlotPointForm({ defaultValues, onSubmit, onCancel, submitLabel = 'Save' }: Props) {
  ...
  return (<form className="entity-form" onSubmit={handleSubmit(onSubmit)}><Input label="Title" placeholder="Plot point title" error={errors.title?.message} required {...register('title')} /><Dropdown label="Status" value={watch('status')} options={PLOT_STATUSES.map(s => ({ label: s, value: s }))} onChange={v => setValue('status', v as PlotPointFormData['status'])} /><Input label="Arc" placeholder="Story arc name" {...register('arc')} /><TextArea label="Description" placeholder="What happens at this plot point..." rows={3} {...register('description')} /><TextArea label="Notes" placeholder="Additional notes..." rows={2} {...register('notes')} /><div className="entity-form__actions"><Button variant="secondary" type="button" onClick={onCancel}>Cancel</Button><Button variant="primary" type="submit" loading={isSubmitting}>{submitLabel}</Button></div></form>);
}
