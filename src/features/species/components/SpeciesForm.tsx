...
export function SpeciesForm({ defaultValues, onSubmit, onCancel, submitLabel = 'Save' }: Props) {
  ...
  return (
    <form className="entity-form" onSubmit={handleSubmit(onSubmit)}>
      ...
      <Input label="Name" placeholder="Species name" error={errors.name?.message} required {...register('name')} />
      <Input label="Habitat" placeholder="Where they live" {...register('habitat')} />
      <TextArea label="Appearance" placeholder="Physical traits..." rows={3} {...register('appearance')} />
      <TextArea label="Culture" placeholder="Society and customs..." rows={2} {...register('culture')} />
      <TextArea label="History" placeholder="Origins and history..." rows={3} {...register('history')} />
      <TextArea label="Abilities" placeholder="Special abilities..." rows={2} {...register('abilities')} />
      <TextArea label="Weaknesses" placeholder="Known weaknesses..." rows={2} {...register('weaknesses')} />
