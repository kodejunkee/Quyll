...
export default function HomePage() {
  ...
  const handleImportProject = async () => {
    try {
      const newProjectId = await pickAndImportQuyllProject();
      if (newProjectId) {
        await loadProjects();
      }
    } catch (error) {
  ...
  const recentProjects = [...projects]
    .filter(p => p.last_opened_at != null)
    .sort((a, b) => {
      const timeA = new Date(a.last_opened_at!).getTime();
      const timeB = new Date(b.last_opened_at!).getTime();
      return timeB - timeA;
    })
    .slice(0, 5);
        ...
        {projects.length === 0 ? (
          <div className="home-empty-container">
            <div className="home-empty">
              ...
              <Button variant="secondary" onClick={handleImportProject} className="home-empty__import-btn">
                <UploadIcon width={16} height={16} style={{ marginRight: '8px' }} />
                Import Existing Quyll Project
              </Button>
            </div>
          </div>
        ) : (
