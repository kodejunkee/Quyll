...
import { ScrollText, Plus, ArrowUpDown, LayoutGrid, Share2 } from 'lucide-react';
...
export default function LorePage() {
  ...
  return (<div className="entity-list-page"><header className="entity-list-page__header"><div><h1 className="entity-list-page__title">Lore</h1><p className="entity-list-page__count">{items.length} entr{items.length !== 1 ? 'ies' : 'y'}</p></div><div className="entity-list-page__actions">
          {viewMode === 'details' && (
            <Button variant="ghost" size="sm" onClick={() => { setSortKey(sortKey === 'title' ? 'created_at' : 'title'); toggleDirection(); }}><ArrowUpDown size={14} />{sortKey === 'title' ? 'Title' : 'Date'} {sortDirection === 'asc' ? '↑' : '↓'}</Button>
          )}
  <Button variant="primary" onClick={() => setCreateOpen(true)}><Plus size={16} />New Lore Entry</Button></div></header>
