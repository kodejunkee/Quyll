...
export function TimelineEventForm({ defaultValues, onSubmit, onCancel, submitLabel = 'Save' }: Props) {
  ...
  return (
    <form className="entity-form" onSubmit={handleSubmit(onSubmit)}>
      ...
      <Input label="Title" placeholder="Event title" error={errors.title?.message} required {...register('title')} />
      <Input label="Date" placeholder="e.g. Year 312, March 15th, etc." {...register('event_date')} />
      <TextArea label="Description" placeholder="What happened..." rows={4} {...register('description')} />
