...
export default function MagicSystemDetailPage() {
  ...
  const sections = [{ label: 'Description', value: entity.description }, { label: 'Rules', value: entity.rules }, { label: 'Limitations', value: entity.limitations }, { label: 'Examples', value: entity.examples }];
  return (<div className="entity-detail"><header className="entity-detail__header"><Button variant="ghost" size="sm" onClick={() => navigate(`/project/${projectId}/magic-systems`)}><ArrowLeft size={16} />Magic Systems</Button><div className="entity-detail__header-actions"><Button variant="secondary" size="sm" onClick={() => setEditOpen(true)}><Edit size={14} />Edit</Button><Button variant="ghost" size="sm" onClick={() => setDeleteOpen(true)}><Trash2 size={14} /></Button></div></header><div className="entity-detail__content" style={{ gridTemplateColumns: '1fr' }}><div className="entity-detail__main"><h1 className="entity-detail__name">{entity.name}</h1>{fields[0]?.value && <Card className="entity-detail__card"><h3 className="entity-detail__card-title">General</h3><div className="entity-detail__fields">{fields.map(({ label, value }) => value ? <div key={label} className="entity-detail__field"><span className="entity-detail__field-label">{label}</span><span className="entity-detail__field-value">{value}</span></div> : null)}</div></Card>}{sections.filter(s => s.value.trim()).map(({ label, value }) => <Card key={label} className="entity-detail__card"><h3 className="entity-detail__card-title">{label}</h3><p className="entity-detail__text">{value}</p></Card>)}</div></div><Modal open={editOpen} onClose={() => setEditOpen(false)} title="Edit Magic System" size="lg"><MagicSystemForm defaultValues={entity} onSubmit={handleUpdate} onCancel={() => setEditOpen(false)} submitLabel="Save Changes" /></Modal><Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)} title="Move to Trash" description={`Move "${entity.name}" to trash?`} confirmLabel="Move to Trash" onConfirm={handleDelete} variant="danger" /></div>);
}
