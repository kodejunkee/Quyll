...
export function MagicSystemForm({ defaultValues, onSubmit, onCancel, submitLabel = 'Save' }: Props) {
  ...
  return (
    <form className="entity-form" onSubmit={handleSubmit(onSubmit)}>
      ...
      <Input label="Name" placeholder="Magic system name" error={errors.name?.message} required {...register('name')} />
      <TextArea label="Description" placeholder="Describe this magic system..." rows={3} {...register('description')} />
