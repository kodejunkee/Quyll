...
export function MagicSystemForm({ defaultValues, onSubmit, onCancel, submitLabel = 'Save' }: Props) {
  ...
  return (<form className="entity-form" onSubmit={handleSubmit(onSubmit)}><Input label="Name" placeholder="Magic system name" error={errors.name?.message} required {...register('name')} /><TextArea label="Description" placeholder="Describe this magic system..." rows={3} {...register('description')} /><TextArea label="Rules" placeholder="How does it work..." rows={3} {...register('rules')} /><TextArea label="Limitations" placeholder="What are the limits..." rows={2} {...register('limitations')} /><Input label="Energy Source" placeholder="What powers it" {...register('energy_source')} /><TextArea label="Examples" placeholder="Notable examples..." rows={2} {...register('examples')} />  <div className="form-row" style={{marginBottom: '1rem'}}>
